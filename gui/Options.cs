exec( "gui/options/Binds" );
exec( "gui/options/Game_General" );
exec( "gui/options/Game_Loadouts" );
exec( "gui/options/HUDs" );
...
function OptionsVideo::validate()
{
   ...
   if ($pref::VideoFullScreenRes == "") $pref::VideoFullScreenRes = Canvas::getDesktopResolution();
...
function OptionsVideo::apply()
{
   ...
   //---------------------------------------------------------------------------   
   // jddNOTE : This was causing unnecessary window resizing in the browser and
   //  doesn't serve any purpose now that there is only 1 device.   Should it be
   //  removed?
   //--------------------------------------------------------------------------- 
   ...
   //   %temp = setFullscreenDevice(MainWindow, "OpenGL");
   //   if (String::ICompare(%temp, "false") != 0)
   //      $pref::VideoFullScreenDriver = "OpenGL";
   //}
