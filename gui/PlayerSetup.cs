...
function PlayerSetupGui::onOpen()
{
   ...
   for(%i = 0; $PCFG::Name[%i] != ""; %i++)
	...
	while (%voiceSet != "")
	{
      ...
      %voiceSet = File::findNext("*.whello.ogg");
	}
	
	%voiceSet = File::findFirst("*.whello.ogg");
	while (%voiceSet != "")
	{
		%strIndex = String::findSubStr(%voiceSet, ".whello.ogg");
		%voiceBase = String::getSubStr(%voiceSet, 0, %strIndex);
	...
	%skinCount = 0;
	%skin = File::findFirst("*.larmor.png");
	while (%skin != "")
	{
		$SkinArray[%skinCount] = %skin;
		%skinCount = %skinCount + 1;
		%skin = File::findNext("*.larmor.png");
	}

	//add all the female skiins to the array
	%skin = File::findFirst("*.lfemale.png");
	while (%skin != "")
	{
		$SkinArray[%skinCount] = %skin;
		%skinCount = %skinCount + 1;
		%skin = File::findNext("*.lfemale.png");
	}
