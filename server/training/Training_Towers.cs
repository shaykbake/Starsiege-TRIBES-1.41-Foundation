...
function TowerSwitch::onCollision(%this, %object)
{
   ...
   if(%playerTeam == 0 )
   {
     ...
     %touchClientName = Client::getName(%playerClient);
   	 %group = "MissionGroup/Towers/tower" @ $currentWay;
	 ...
	 schedule("message::bottomprint(" @ %playerClient @ ", \"<f1><jc>Objective " @ $currentWay @ " has been accomplished!\", 5);", %time);
   ...
   {
     message::bottomprint(%playerClient, "<f1><jc>The enemy has taken one of your towers!", 5);
     schedule("message::bottomprint(" @ %playerClient @ ", \"<f1><jc>You must go back and claim the tower before you can win the mission!\", 5);", 5);
	 message::All(0, "~wCapturedTower.wav");
	 
	 //set all objects to opposite team , Ai's Team.
     %group = nameToId("MissionGroup/Towers/tower" @ %switchNum);
     for(%i = 0; (%obj = Group::getObject(%group, %i)) != -1; %i++)
     {  
...
//message::bottomprints to all players on teamId
function Teambottomprint(%teamId, %string)
{
  for(%i = 0; %i < getNumClients(); %i++)
  {
	...
	{ 
      message::bottomprint(%player, %string, 5);
    }
...
function Towers::teamMissionObjectives(%towNum, %taken)
{
   ...
   if(%towNum == 1)
   {  
	  Team::setObjective(0, 15, "<f1><Btower_teamcontrol.png>\n   " @ %string);
   }
   else if(%towNum == 2) 
   {
      Team::setObjective(0, 16, "<f1><Btower_teamcontrol.png>\n   " @ %string);
   } 						  
   else if(%towNum == 3) 
   {
      Team::setObjective(0, 17, "<f1><Btower_teamcontrol.png>\n   " @ %string);
   }
   //initial setup
   else
   {
      Team::setObjective(0, 15, "<f1><Btower_enemycontrol.png>\n   Tower 1 has not been captured.");
   	  Team::setObjective(0, 16, "<f1><Btower_enemycontrol.png>\n   Tower 2 has not been captured.");
	  Team::setObjective(0, 17, "<f1><Btower_enemycontrol.png>\n   Tower 3 has not been captured.");
   }
}
...
function TowersTraining::setWayPoint(%clientid)
{
   ...
   message::bottomprint(%clientId, "<f1><jc>Your Waypoint has been set to tower " @ $currentWay @ ".", 5);    
  ...
  if($numToComplete != 0)
  {
    if(%this == $SwitchObject[$currentWay] && !%taken)
    {
      if($currentWay != 3)
	  {
        ...
        message::bottomprint(%playerId, "<f1><jc>Your Waypoint has been set to tower " @ $currentWay @ ".", 5);
...
function setUpLastWayPoint()
{
  ...
  %currentx = getWord(%newPos, 0);
  %currenty = getWord(%newPos, 1);
  message::bottomprint(%playerId, "<f1><jc>Your Waypoint has been set back to tower " @ $currentWay @ " for recapture.", 5);
  issueCommand(%playerId, %playerId, 0, "Goto waypoint " @ $currentWay, %currentx, %currenty);
}
...
{
  %cl = client::getFirst();
  message::bottomprint( %cl, "<f1><jc>You must take the tower that has been set by your commander first.", 5);
}
...
{
  schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Training Mission 6 - Capture and Hold.\", 5);", 0);
  schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>This mission will introduce you to the capture and hold game scenario.\", 5);", 5);
  schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Bring up The Objectives screen for a gameplay description.\", 5);", 10);
