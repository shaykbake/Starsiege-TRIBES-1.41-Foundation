...
{
   message::bottomprint(%clientId, "<jc><f1>Training Mission 7 - Find and Retrieve", 5);
   schedule("message::bottomprint(" @ %clientId @ ", \"<f1><jc>This mission will introduce you to the Find and Retrieve game scenario.\", 5);", 5);
...
function FlagStand::onCollision(%this, %object)
{
   ...
   {   
	  ...
	  if($captures != 3)
	  {  
	     message::bottomprint(%playerClient, "<jc><f1>You conveyed the " @ %object.flag.objective @ " to your base.", 5);
...
function Flag::onCollision(%this, %object)
{
   ...
   if(!$flagDropped)
     message::bottomprint(%playerClient, "<jc><f1>You took the " @ %this.objective @ " !", 5);
   else
   {  
     message::bottomprint(%playerClient, "<jc><f1>You recovered the " @ %this.objective @ " !", 5);
	 %this.lost = "false";
   }
...
function Flag::onDrop(%player, %type)
{
   ...
   message::bottomprint(%playerClient, "<jc><f1>You dropped the " @ %flag.objective @ " !", 5);
...
function Retrieval::checkFlagReturn(%this, %clientId)
{
   if(%this.lost)
   {
      message::bottomprint(%clientId, "<f1><jc>" @ %this.objective @ " was returned to its base.", 5);
...
function retrieval::setWayPoint(%flag, %cl, %init)
{
   ...
   if(%flag.lost)
   {
      schedule("message::bottomprint(" @ %cl @ ", \"<jc><f1>Waypoint set to " @ %flag.objective @ "\", 5);", 5);
	  issueCommand(%cl, %cl, 0, "Waypoint set to " @ %flag.objective, %flagx, %flagy);
   }
   else	if(%flag.conveyed || %init)
   {
      schedule("message::bottomprint(" @ %cl @ ", \"<jc><f1>Waypoint set to next objective\", 5);", 5);
	  issueCommand(%cl, %cl, 0, "Waypoint set to next objective", %flagbasex, %flagbasey);
   }
   else if(%player.hasFlag)
   {
      schedule("message::bottomprint(" @ %cl @ ", \"<jc><f1>Waypoint set back to your base\", 5);", 5);
