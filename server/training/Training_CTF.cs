...
function ctf::init()
{
  ...
  %group0 = "MissionGroup/teams/team0/flag";
  %group1 = "MissionGroup/teams/team1/flag";
...
function CTFTraining::setWayPoint(%this, %init)
{
   ...
   if(!$flagDropped)
   {
     if(!%init)
     {
       ...
       {  
         for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
         {
           ...
           schedule("message::bottomprint( " @ %cl @ ", \"<jc><f1>WayPoint set to enemy base.\", 5);", 5);
         ...
         for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
     	   {
     	     ...
     	     schedule("message::bottomprint(" @ %cl @ ", \"<jc><f1>Bring enemy flag back to your base for a capture.\", 5);", 5);
     	   }
     	 }
     }
     else
     {  
         ...
         schedule("message::bottomprint(" @ %clFirst @ ", \"<jc><f1>WayPoint set to enemy base.\", 5);", 15);
     }
   }
   else
   {  
     issueCommand(%clFirst, %clFirst, 0, "Make your way to the enemy's flag.", %currentx, %currenty);
     schedule("message::bottomprint(" @ %clFirst @ ", \"<jc><f1>WayPoint set to dropped flag.\", 5);", 5);
   }
}       
...
function Flag::onDrop(%player, %type)
{
   ...
   message::bottomprint(%playerClient, "<f1><jc>You dropped the " @ getTeamName(%flagTeam) @ " flag!", 5);
...
function CTF::checkFlagReturn(%team, %sequenceNum)
{
   ...
   {
      message::all(0, "The enemy flag was returned to thier base.~wflagreturn.wav");
   ...
   if(getObjectType(%object) == "Player")
   {
      ...
      if(%flagTeam == %playerTeam)
      {
         ...
         if(!$flagAtHome[%playerTeam])
         {
            ...
            message::bottomprint(%playerClient, "<jc><f1>You returned the " @ getTeamName(%playerTeam) @ " flag!" @ "~wflagreturn.wav", 5);
            CTF::checkMissionObjectives();
         ...
         else
         {
            ...
            {
               message::bottomprint(%playerClient, "<jc><f1>You captured the " @ getTeamName(%enemyTeam) @ " flag!", 5);
               message::all(%playerClient, "~wflagcapture.wav");
	  ...
	  else
      {
         ...
         if(!$flagDropped)
           message::bottomprint(%playerClient, "<jc><f1>You took the " @ getTeamName(%enemyTeam) @ " flag!", 5);
		 else
		   message::bottomprint(%playerClient, "<jc><f1>You recovered the " @ getTeamName(%enemyTeam) @ " flag!", 5);
         
         message::all(%playerClient, "~wflag1.wav");
...
function Player::leaveMissionArea(%this)
{
   ...
   if(%flag != -1)
   {
      ...
      message::bottomprint(%playerClient, "<f1><jc>You left the mission area while carrying the " @ getTeamName(1) @ " flag!", 5);
...
function Game::initialMissionDrop(%clientId)
{
  ...
  ctf::init(%clientId);
  schedule("message::bottomprint(" @ %clientId @ ", \"<f1><jc>Training Mission 5 - Capture the Flag.\", 5);", 0);
  schedule("message::all(0, \"~wshell_click.wav\");", 0);
  schedule("message::bottomprint(" @ %clientId @ ", \"<f1><jc>This mission will introduce you to the capture the flag game scenario.\", 5);", 5);
  schedule("message::all(0, \"~wshell_click.wav\");", 5);
  schedule("message::bottomprint(" @ %clientId @ ", \"<f1><jc>Bring up The Objectives screen for a gameplay description.\", 5);", 10);
