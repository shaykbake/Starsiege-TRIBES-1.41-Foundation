...
{   
   //weapon discription
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>This mission will introduce you to the many weapons that are available in TRIBES.\", 10);", 5);
   schedule("message::all(0, \"~wshell_click.wav\");", 5);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Ready?\", 10);", 15);
   schedule("message::all(0, \"~wshell_click.wav\");", 15);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Let's start with the blaster.  This is an energy weapon which all armor classes can carry.  It is a good close- to medium-range weapon that feeds off of your energy supply.\", 10);", 25);
   schedule("message::all(0, \"~wshell_click.wav\");", 25);
   
   //mount weapons
   schedule("Player::setItemCount(" @ %playerId @ ",Blaster, 1);", 25);
   schedule("Player::mountItem(" @ %playerId @ ", Blaster, 0);", 25);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Time to do some shooting!  Somewhere around you a player is trying to infiltrate your base.  Eliminate him with the blaster.\", 10);", 35);
...
{   
   ...
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Next is the plasma gun.\", 10);", 10);
   schedule("message::all(0, \"~wshell_click.wav\");", 10);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>The plasma gun has about the same range as the blaster, but far more punch.  This weapon is a good choice against slower heavy armors because of its splash damage radius. \", 10);", 20);
   schedule("message::all(0, \"~wshell_click.wav\");", 20);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Let's take out a heavy armor and see how you like it.\", 10);",30);
   schedule("message::all(0, \"~wshell_click.wav\");", 30);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Once again, an enemy tribesman has infiltrated your base, and it is up to you to dispose of the threat.\", 10);", 40);
   schedule("message::all(0, \"~wshell_click.wav\");", 40);
...
{   
   //weapon discription
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Next, the chain gun.  Capable of churning out explosive flechettes in rapid order, it can chew up even heavy armors in short order.\", 10);", 10);
   schedule("message::all(0, \"~wshell_click.wav\");", 10);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>It can be carried by all armor classes.  Deadly at short range, it loses much of its effectiveness at greater distances.\", 10);", 20);
   schedule("message::all(0, \"~wshell_click.wav\");", 20);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Let's fire a few rounds and get an idea of the range of the chain gun. Try and take out the target that is crossing your path below your base tower.\", 10);", 30);
   schedule("message::all(0, \"~wshell_click.wav\");", 30);
...
{   
   //discribe the weapon
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>The next weapon available to you is the disk launcher.\", 10);", 10);
   schedule("message::all(0, \"~wshell_click.wav\");", 10);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>This is definitely a weapon that you want to master.  Little else in tribal arsenals can match the damage dealt by its magnetic spinfusor disks. This weapon also has a wide splash radius, so aim at the feet of your enemy for maximum effectivness.\", 10);", 20);
   schedule("message::all(0, \"~wshell_click.wav\");", 20);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>The best way to get a feel for this weapon is to shoot some discs.  See if you can find an enemy tribesman wandering around your tower, and take 'em out!\", 10);", 30);
   schedule("message::all(0, \"~wshell_click.wav\");", 30);
...
{   
   //discribe weapon
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>For long-range sniping, nothing beats the laser rifle.  A powerful energy-based weapon, it can hit a target kilometers away!\", 10);", 5);
   schedule("message::all(0, \"~wshell_click.wav\");", 5);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Because of its high energy consumption, you must also have an energy pack to use this weapon and it can only be used by light armors.\", 10);", 15);
   schedule("message::all(0, \"~wshell_click.wav\");", 10);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>The zoom feature enhances the value of the laser rifle immensely. The default zoom key is E. To change the magnification, press the Z key.  You can change the magnification to two, five, ten or even twenty times normal vision!\", 10);", 25);
   schedule("message::all(0, \"~wshell_click.wav\");", 25);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Since a head shot deals more damage than a shot to other parts of the body, being able to use the zoom feature quickly and accurately can make the difference between killing or being killed.\", 10);", 35);
   schedule("message::all(0, \"~wshell_click.wav\");", 35);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Zoom in on an enemy tribesmen and take him out. Use your compass to locate the enemy to the north of you, then take him our with the laser rifle.\", 10);", 45);
   schedule("message::all(0, \"~wshell_click.wav\");", 45);
...
{   
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>On to a more explosive weapon!  Let's take a look at the grenade launcher.  It can be carried by all armor classes.\", 10);", 5);
   schedule("message::all(0, \"~wshell_click.wav\");", 5);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Grenades launched from the launcher have a timed fuse, exploding a few seconds after impact.  To clear out the inside of a bunker, just lob a couple of grenades insde and watch the enemy come running out.\", 10);", 15);
   schedule("message::all(0, \"~wshell_click.wav\");", 15);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>An enemy tribesman is thinking of trying to hide in a bunker.  Time to teach him the error of his ways.\", 10);", 25);
   schedule("message::all(0, \"~wshell_click.wav\");", 25);
   schedule("message::bottomprint(" @ %playerId @ ", \"<f1><jc>Watch the bunker at the bottom of your base tower, and fire a couple of grenades when you see the enemy tribesman run inside.\", 10);", 35);
   schedule("message::all(0, \"~wshell_click.wav\");", 35);
...
function AI::onDroneKilled(%aiName)
{
  ...
  if(%aiName == "Nailz")
  {
	...
	message::bottomprint(%clientId, "<f1><jc>Good Job!", 5);
	$AIkilled++;
	ObjectiveScreen();
  }
...
function endMission(%clientId)
{
  ...
  schedule("message::bottomprint(" @ %clientId @ ", " @ %String @ " , 10);", 5);
  schedule("message::all(0, \"~wshell_click.wav\");", 5); 
...
{ 
	...
	message::all(0, "You must Stay inside your base!" @ "~wshieldhit.wav");
...
function Game::initialMissionDrop(%clientId)
{
   ...
   schedule("message::bottomprint(" @ %clientId @ ", \"<f1><jc>Training Mission 2 - Weapon Introduction.\", 5);", 0);
	...
	if($matchStarted) {
		...
		if(%delta > 0) {
      	...
      	addToSet("MissionCleanup", %obj);
			...
			message::all(0, "~wmale2.wdsgst2.wav");
