...
function DM::missionObjectives()
{
   ...
   if(!$Server::timeLimit)
	...
	for(%l = -1; %l < 1 ; %l++) {		
		%lineNum = 0;
		if($timeReached == "") {
	 	  	Team::setObjective(%l, %lineNum, "<jc><B0,0:deathmatch.png>");
	      ...
	      Team::setObjective(%l, %lineNum++, " ");
			...
			while(%i < %numClients && %clientList[%i].ratio != 0 && (%TopRatio == "" || (%TopRatio ==  (%clientList[%i+1]).ratio && %TopRatio != 0) )) {
	 	  		Team::setObjective(%l, %lineNum++, "<L14><f5><Bskull_big.png>\n" @ String::escapeFormatting(Client::getName(%clientList[%i])) @ "<f1> with a ratio of <f5>" @ (%clientList[%i]).ratio @ ".0%");
	   ...
	   //print out top 5 scores
		...
		while(%index < %numClients && %clientList[%index].ratio != 0 && (%index < 5 || (%clientList[%index].ratio == %lastRatio && %lastRatio != 0))) {
	  	   ...
	  	   Team::setObjective(%l, %lineNum++,"<Bskull_small.png>" @ String::escapeFormatting(Client::getName(%clientList[%index])) @ " <L31>" @ (%clientList[%index]).scoreKills @ "<L53>" @ (%clientList[%index]).scoreDeaths @ "<L72>" @ (%clientList[%index]).ratio @ ".0%");
