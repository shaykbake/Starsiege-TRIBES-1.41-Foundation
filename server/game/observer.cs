...
function Observer::triggerUp(%client) {
   ...
   else if(%client.observerMode == "pregame" && $Server::TourneyMode)
   {
      ...
      {
         %client.notready = "";
         message::all(0, Client::getName(%client) @ " is READY.");
         if(%client.notreadyCount < 3)
            message::bottomprint(%client, "<f1><jc>Waiting for match start (FIRE if not ready).", 0);
         else 
            message::bottomprint(%client, "<f1><jc>Waiting for match start.", 0);
      }
      else
      {
         ...
         if(%client.notreadyCount < 4)
         {
            ...
            message::all(0, Client::getName(%client) @ " is NOT READY.");
            message::bottomprint(%client, "<f1><jc>Press FIRE when ready.", 0);
         }
         return;
      }
...
function Observer::enterObserverMode(%clientId)
{
   ...
   Event::Trigger( eventServerClientJoinTeam, %clientId, -1 );
...
function Observer::setTargetClient(%client, %target)
{
   ...
   message::bottomprint(%client, "<jc>Observing " @ String::escapeFormatting(Client::getName(%target)), 5);
