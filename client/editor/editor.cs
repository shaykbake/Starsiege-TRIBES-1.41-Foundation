...
function SeedTerrain::onAction()
{
   ...
   exec("client/editor/"@ %world @ ".terrain.cs");
...
function ME::init()
{
   ...
   if($ME::Loaded == false)
   {
      %obj = newObject("Prefabs", SimVolume, "prefabs.zip");
      addToSet("MissionCleanup", %obj);
      
      $Server::timeLimit = 0;
      $ME::Loaded = true;
      exec("client/editor/move.cs" );
      ME::Move();
      exec("client/editor/med.cs");
      exec("client/editor/ted.cs");
      exec("client/editor/terrains.cs");
      exec("client/editor/RegisterObjects.cs");

      // execute the user's stuff - do here to allow overriding of ted/med variables
      exec("client/editor/editorconfig.cs");
      ted();
      MEMode();
   }
}
