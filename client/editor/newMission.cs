...
{
	// Mission Object list kept trying to rebuild groups that didn't exist and crashing on new missions
   ...
   %worldVolName = strcat(%worldName, "World", ".zip");
   %terrVolName = strcat(%worldName, "Terrain", ".zip");
   ...
   newObject("InterfaceVol", SimVolume, "interface.zip");
   ...
   for(%i = 0; %i < $NM::numTeams; %i++)
   {
      ...
      newObject("DropPoints", SimGroup);
	   ...
	   addToSet("MissionGroup/Teams/team" @ %i, DropPoints);
   }
...
GuiPushDialog(MainWindow, "gui/newmission.gui");
NewMission::reset();
